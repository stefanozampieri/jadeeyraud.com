...
function updateZodiac() {
  ...
  const { animal, element } = getChineseZodiac(1989);
  const stone = 'Peridot';
  const el = {
    westernSign: document.getElementById('western-sign'),
    chineseSign: document.getElementById('chinese-sign'),
...
async function buildTimeline() {
  ...
  try {
    ...
    const now = new Date();
    const currentYear = now.getFullYear();
    // last birthday year is currentYear if we've passed Aug 15, else currentYear - 1
    const lastYear = (new Date(currentYear, 7, 15) <= now) ? currentYear : currentYear - 1;
    for (let y = 1989; y <= lastYear; y++) years.push(y);

    const items = [];
    for (const y of years) {
      const d = new Date(y, 7, 15); // Aug 15 y
...
// Click-to-pop random images (jade1/2.png)
(function setupClickPops() {
  const images = ['jade1.png', 'jade2.png'];
  function pick() {
